...
module bp_be_dcache
 ...
 #(parameter bp_params_e bp_params_p = e_bp_inv_cfg
  ...
  for (genvar i = 0; i < lce_assoc_p; i++) begin: data_mem
   ...
   // miss_detect
  //
  logic [lce_assoc_p-1:0] tag_match_tl;
  logic [lce_assoc_p-1:0] load_hit_tl;
  logic [lce_assoc_p-1:0] store_hit_tl;
  logic load_hit_v_tl;
  ...
  wire load_miss_tl = ~load_hit_v_tl & tv_we & load_op_tl_r & ~uncached_i;
  wire store_miss_tl = ~store_hit_v_tl & tv_we & store_op_tl_r & ~uncached_i & ~sc_op_tl_r;
  wire lr_miss_tl = tv_we & lr_op_tl_r & ~store_hit_v_tl;

  wire miss_tl = load_miss_tl | store_miss_tl | lr_miss_tl;
  ...
  logic miss_tv_r;
  logic lr_miss_tv_r;
  logic store_miss_tv_r;
  logic load_miss_tv_r;
  ...
  logic [lce_assoc_p-1:0] load_hit_tv_r;
  logic [lce_assoc_p-1:0] store_hit_tv_r;
  logic load_hit_v_tv_r;
  logic store_hit_v_tv_r; 
  ...
  always_ff @ (posedge clk_i) begin
	...
	store_hit_tv_r <= store_hit_tl;
        load_hit_v_tv_r <= load_hit_v_tl;
        store_hit_v_tv_r <= store_hit_v_tl;
	miss_tv_r <= miss_tl;
        lr_miss_tv_r <= lr_miss_tl;
        store_miss_tv_r <= store_miss_tl;
        load_miss_tv_r <= load_miss_tl;
 ...
 //
  ...
  assign lr_hit_tv = v_tv_r & lr_op_tv_r & store_hit_v_tv_r;
  // Succeed if the address matches and we have a store hit
  assign sc_success  = v_tv_r & sc_op_tv_r & store_hit_v_tv_r & load_reserved_v_r 
  ...
  always_comb begin
    ...
    if(load_miss_tv_r) begin
      cache_req_cast_o.msg_type = e_miss_load;
      cache_req_v_o = cache_req_ready_i;
    end
    else if(store_miss_tv_r | lr_miss_tv_r) begin
  ...
  assign v_o = v_tv_r & ((uncached_tv_r & (load_op_tv_r & uncached_load_data_v_r))
                         ...
                         | (~uncached_tv_r & ~fencei_op_tv_r & ~miss_tv_r)
                         );
  ...
  assign stat_mem_w_li = (v_tv_r & ~uncached_tv_r)
    ? ~(load_miss_tv_r | store_miss_tv_r | lr_miss_tv_r)
  ...
  // write buffer
  //
  assign wbuf_v_li = v_tv_r & store_op_tv_r & store_hit_v_tv_r & ~sc_fail & ~uncached_tv_r; 
  ...
  always_ff @ (posedge clk_i) begin
    if (reset_i) begin
      ...
      if (lr_op_tv_r & v_o & ~lr_miss_tv_r) begin
  ...
  always_ff @ (negedge clk_i) begin
    if (v_tv_r) begin
      assert($countones(load_hit_tv_r) <= 1)
        else $error("multiple load hit: %b. id = %0d. addr = %H", load_hit_tv, cfg_bus_cast_i.dcache_id, addr_tag_tv_r);
      assert($countones(store_hit_tv_r) <= 1)
        else $error("multiple store hit: %b. id = %0d. addr = %H", store_hit_tv, cfg_bus_cast_i.dcache_id, addr_tag_tv_r);
      assert (~(sc_op_tv_r & load_reserved_v_r & (load_reserved_tag_r == addr_tag_tv) & (load_reserved_index_r == addr_index_tv)) | store_hit_v_tv_r)
